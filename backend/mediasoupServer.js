  ...
  const createConsumers = async () => {
    try {
      ...
      const consumerTransport = await router.createWebRtcTransport({
        listenIps: [{ ip: '192.168.0.34', announcedIp: null }],
        enableUdp: true,
        enableTcp: true,
        preferUdp: true
      });
...
app.get('/webrtc-stream/:roomId', (req, res) => {
  ...
  res.json({
    ...
    webrtcUrl: `ws://192.168.0.34:4000`,
...
const PORT = 4000;
server.listen(PORT, '192.168.0.34', () => {
  console.log(`MediaSoup server running on http://192.168.0.34:${PORT}`);
}); 